...
async def update_metadata(lanraragi: LRRClient, metadata_service: MetadataService):
    ...
    logger.info(f"[update_metadata] {len(tasks)} archives updated.")

async def update_metadata_from_plugin(lanraragi: LRRClient, database: DatabaseService, namespace: str, retry_ok: bool=False):
    ...
    async def __handle_task(task: Tuple[str, str, int, float, int], task_i: int, total_tasks: int):
        ...
        time_to_sleep = random.random() * 5 # sleep with delay
        await asyncio.sleep(time_to_sleep)
    ...
    if retry_ok:
        ...
        for i, task in enumerate(ok_tasks):
            await __handle_task(task, i, num_ok_tasks)
    ...
    for i, pending_metadata_task in enumerate(pending_metadata_tasks):
        await __handle_task(pending_metadata_task, i, num_pending_tasks)
    ...
    for i, failed_task in enumerate(failed_metadata_tasks):
        await __handle_task(failed_task, i, num_failed_tasks)
    logger.info(f"[metadata_plugin_{namespace}] Finished metadata update.")