...
from satellite.utils.image import archive_contains_incomplete_image
...
def __analyze_archive(path_str: str) -> ArchiveAnalysisResponse:
    ...
    _path = Path(path_str)
    try:
        if archive_contains_incomplete_image(_path):
...
async def scan_lrr_archives(contents_dir: Path, database: DatabaseService, num_workers: int, batch_size: int, lock: RWLock):
    ...
    async with lock.reader_lock:
        ...
        all_archives = discover_all_archives_in_folder(contents_dir)
        archive_paths = []
